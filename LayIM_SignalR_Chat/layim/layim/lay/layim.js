...
; !function (win, undefined) {
        ...
        cache: {
            ...
            set: function (val) {
                if (typeof val === 'object') {
                    localStorage.setItem("layim_current_user", JSON.stringify(val));
                } else if (typeof val === 'string') {
                    localStorage.setItem(val);
                }
            }
        },
        user: (function () { //当前用户信息
    ...
    xxim.popchat = function (param) {
        ...
        log.html = '<div class="layim_chatbox" id="layim_chatbox">'
                ...
                + '       <div class="upload"> <a href="javascript:;"><i class="layim_addimage" title="上传图片"><form id="filePostImage" action="/Home/UploadFile" method="post" enctype="multipart/form-data"><input type="file" name="file" class="upload-input" accept="image/*" onchange="save(1)" value="" data-type="jpg,gif,png,bmp,jpeg" data-size="50" /></form></i></a>'
                + '        <a href="javascript:;"><i class="layim_addfile" title="上传附件"><form id="filePostFile" action="/Home/UploadFile" method="post" enctype="multipart/form-data"><input type="file" class="upload-input" name="file" onchange="save(2)" value="" /></form></i></a></div>'
        ...
        //id="layim_area' + param.type + param.id +
        //id="layim_group' + param.type + param.id
        //滚动加载历史记录
        var one = $('#layim_area' + param.type + param.id);
        var group = $('#layim_area' + param.type + param.id);
        //单条
        if (one.length&&param.type=='one') {
            one.scroll(function () {
                ...
                if (nScrollTop == 0 && one.data('load') != 1) {
                    //顶部加载历史记录GetHistory(string lastid,string rid,string type, int groupid)
                    config.json(config.api.history, { lastid: lastid, rid: param.id, type: param.type, groupid: config.user.id }, function (result) {
                        xxim.loadhistory(one, result);
                        if (result.length == 0) {
        ...
        //群组
        if (group.length&&param.type=='group') {
            group.scroll(function () {
                var nScrollTop = $(this)[0].scrollTop;
                var lastid = group.find('li').first().data('msgid') || '';
                if (nScrollTop == 0 && group.data('load')!=1) {
                    //顶部加载历史记录
                    config.json(config.api.history, { lastid: lastid, rid: param.id, type: param.type, groupid: param.id }, function (result) {
                        xxim.loadhistory(group, result);
                        if (result.length == 0) {
                            group.data('load', 1);//加载完毕
                        }
                    });
                }
            });
    ...
    //ajax加载历史记录
    xxim.loadhistory = function (obj,msgs) {
        var l = {};
        l.html = function (param, type) {
        ...
        $(msgs).each(function (i, result) {
            var type = result.fromuser.id == currentUser.id ? "me" : "";
    ...
    xxim.popchatbox = function (othis) {
        ...
        if (!config.chating[key]) {
            console.log("没有弹框");
            //单聊连接服务器
            csClient.server.ctoc(config.user.id, param.id, param.type);
            xxim.popchat(param);
            config.chatings++;
        } else {
            console.log("有弹框");
    ...
    xxim.transmit = function () {
        ...
        log.send = function () {
            ...
            if (data.content.replace(/\s/g, '') === '') {
                ...
                */
                csClient.server.send(data.content, config.user.id, config.user.name, config.user.face, data.id, xxim.nowchat.type);
            }
    ...
    xxim.getDates = function (index) {
        ...
        config.json(api[index], { userid: config.user.id }, function (datas) {
            console.log(datas);
            if (datas.status === 1) {
    ...
    xxim.view = (function () {
        var xximNode = xxim.layimNode = $('<div id="xximmm" class="xxim_main">'
                ...
                + '  <div class="xxim_tabs" id="xxim_tabs"><span class="xxim_tabfriend" title="好友"><i></i></span><span class="xxim_tabgroup" title="群组"><i></i></span><span class="xxim_latechat"  title="好友申请"><i></i></span></div>'
                ...
                //
                + '<li class="xxim_seter" id="xxim_seter" title="更换头像">'
                    + '<i></i>'
                    + '<div class="upload">'
                    + '<form id="filePostUserPhoto" action="/Home/UploadFile" method="post" enctype="multipart/form-data"><input type="hidden" value="'+config.user.id+'" id="hidden_current_user_id" name="userid"><input type="file" name="file" class="upload-input" accept="image/*" onchange="save(3)" value="" data-type="jpg,gif,png,bmp,jpeg" data-size="50" /></form>'
                    + '</div>'
                + '</li>'
